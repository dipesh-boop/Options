...
### OP-003 — Multi-leg order quantity has no cross-leg equality validator
- **Severity:** MEDIUM (currently unreached in production)
- **Component:** `src/brokers/base.py:61-73` (`OrderLeg`); `src/brokers/paper.py:393` (`attempt_fill`), `:530-552` (`_apply_fill`)
- **Description:** `OrderLeg.quantity` is a per-leg field with no validator anywhere enforcing all legs of one order agree. `PaperBroker.attempt_fill` consults only `order.legs[0].quantity` to determine remaining size and terminal status, and applies the one computed fill quantity to every leg identically. The one production call site that builds these (`src.risk.engine._build_approved_order`) always constructs every leg with the same `num_contracts`, so this cannot currently produce a lopsided position — but nothing enforces that invariant at the type level.
- **Possible consequence:** a future code path (or a data-entry bug) that produced mismatched leg quantities (e.g. `[short qty=2, long qty=5]`) would pass model validation and have the mismatch silently discarded — PaperBroker would fill both legs at leg 0's quantity with no error, no rejection, no log of the discrepancy.
- **Reproduction:** `PlaceOrderRequest(legs=[OrderLeg(..., quantity=2), OrderLeg(..., quantity=5)], ...)` constructs and fills without error, using quantity 2 for both legs.
- **Recommended remediation (not applied):** add a validator on `PlaceOrderRequest` (or `ApprovedOrder`) requiring all legs to share the same quantity, or explicitly support genuinely different per-leg ratios if that's ever a real strategy shape.
