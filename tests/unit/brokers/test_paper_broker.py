...
from src.brokers.base import OrderAction, OrderLeg, OrderStatus, PlaceOrderRequest, SqliteIdempotencyStore
...
class TestIdempotencySurvivesRestartRegressionSY002:
    """SY-002: a `PaperBroker` wired to the default
    `InMemoryIdempotencyStore` cannot recognize a retry after a process
    restart -- a fresh broker instance sees no history at all and would
    fill a retried order a second time. Wiring in `SqliteIdempotencyStore`
    closes that gap: a *brand-new* broker instance, pointed at the same
    file, still recognizes the same `client_order_id` and returns the
    original fill instead of creating a duplicate."""

    @pytest.mark.asyncio
    async def test_sqlite_backed_broker_recognizes_a_retry_after_simulated_restart(self, tmp_path):
        db_path = tmp_path / "idempotency.db"

        first_process_broker = PaperBroker(
            initial_cash=1_000_000.0, config=PaperBrokerConfig(fill_model=FillModel.MID),
            idempotency_store=SqliteIdempotencyStore(db_path), now=NOW,
        )
        await first_process_broker.connect()
        first_process_broker.update_market_data(make_pcs_chain())
        first_fill = await first_process_broker.place_order(pcs_request())
        assert first_fill.status == OrderStatus.FILLED
        del first_process_broker  # simulate the process crashing

        second_process_broker = PaperBroker(
            initial_cash=1_000_000.0, config=PaperBrokerConfig(fill_model=FillModel.MID),
            idempotency_store=SqliteIdempotencyStore(db_path), now=NOW,
        )
        await second_process_broker.connect()
        second_process_broker.update_market_data(make_pcs_chain())
        # A retry after the "crash" with the *same* client_order_id must
        # be recognized as a duplicate -- not filled a second time.
        retried = await second_process_broker.place_order(pcs_request())
        assert retried.broker_order_id == first_fill.broker_order_id
        assert retried.status == OrderStatus.FILLED
        # No fills were generated by the second broker instance -- the
        # retry never re-entered the fill logic at all.
        assert await second_process_broker.get_fills() == []

    @pytest.mark.asyncio
    async def test_in_memory_backed_broker_by_contrast_would_refill_after_a_restart(self):
        """Documents exactly the gap the sqlite-backed store closes: a
        fresh in-memory-backed broker (the default) has no memory of an
        order a prior instance placed, so an identical retry is treated
        as brand new and refilled."""
        first_process_broker = await _broker(fill_model=FillModel.MID)
        first_process_broker.update_market_data(make_pcs_chain())
        await first_process_broker.place_order(pcs_request())
        del first_process_broker

        second_process_broker = await _broker(fill_model=FillModel.MID)
        second_process_broker.update_market_data(make_pcs_chain())
        retried = await second_process_broker.place_order(pcs_request())
        assert retried.status == OrderStatus.FILLED
        # Unlike the sqlite-backed case, this second, independent broker
        # instance actually re-ran the fill logic and generated new fills.
        assert len(await second_process_broker.get_fills()) == 2
