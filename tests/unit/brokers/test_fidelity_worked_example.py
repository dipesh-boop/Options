...
class TestGeneratedTicketMatchesTheExample:
    ...
    def test_rendered_text_contains_every_key_figure_from_the_example(self):
        ...
        assert "SPY PUT CREDIT SPREAD" in text
        assert "10/16/2026" in text
        assert "SELL TO OPEN:" in text
        assert "2 SPY 620 PUT" in text
        assert "BUY TO OPEN:" in text
        assert "2 SPY 615 PUT" in text
        assert "NET CREDIT LIMIT" in text
        assert "$1.35" in text
        assert "$270" in text
        assert "$730" in text
        assert "$618.65" in text
        assert "$0.68" in text
        assert "21 DTE" in text
